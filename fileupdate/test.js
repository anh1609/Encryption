var pl = document.getElementById("plaintext");
var keya = document.getElementById("key");
var ans = document.getElementById("answer");
var r = [];
var title = []
for (let i = 0; i < 16; i++) {
    r.push(document.getElementById(`round-${i+1}`));
}
var inp = document.getElementById("input");
var ip = document.getElementById('ip-right')
...
// đỏi từ hex sang nhị phân
function hextobin(hex) {
    // hex = "abcd"
    // chuyển đổi đầu vào thành chuỗi,loại bỏ tiền tố 0x 
    hex = hex.toString()
...
// console.log(hextobin())
// đổi từ nhị phân sang hex
...
// hàm hoán vị charAt  trả về ký tự tại một vị trí được chỉ định trong một chuỗi
...
// parseInt  phân tích một giá trị dưới dạng chuỗi và trả về số nguyên đầu tiên.
function xor(a, b) {
    while (a.length < b.length) {
        a = "0" + a /*đảm bảo độ dài 2 chuỗi bằng nhau nếu k thì thêm số 0 */
    }
    var res = "",
        i = a.length,
        j = b.length;
...
// dịch trái input là 1 chuỗi ,mỗi kí tự được mã hóa thành 1 byte
...
function sBox(input) {
    ...
    for (var i = 0; i < 48; i += 6) {
        ...
        output += sbox[num][row][col].toString(16); /*toString(16) đẻ chuyển lại sang hex */
    }
    return output;
}
...
function round(input, key, num) {
    ...
    // xor
    left = xor(left, temp);
    r[num].innerHTML = `L=${right.toUpperCase()}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;R=${left.toUpperCase()}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;K=${key.toUpperCase()}`;
...
function encrypt(plaintext, key) {
    ...
    ip.innerHTML = plaintext.toUpperCase();
    split.innerHTML = `L=${plaintext.substring(0, 8).toUpperCase()}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;R=${plaintext.substring(8, 16).toUpperCase()}`;
...
function runEn() {
    ...
    ans.innerHTML = encrypt(pl.value, keya.value);
    inp.innerHTML = pl.value;
    result.innerHTML = encrypt(pl.value, keya.value);
...
function runDe() {
    ans.innerHTML = decrypt(pl.value, keya.value);
    inp.innerHTML = pl.value;
    result.innerHTML = decrypt(pl.value, keya.value);
    for (let i = 0; i < title.length; i++) {
        title[i].style.display = "block";
    }
    img.style.display = 'none';
